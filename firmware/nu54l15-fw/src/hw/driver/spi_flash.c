...
bool spiFlashEraseBlock(uint32_t block_addr)
{
  ...
  ret &= spiFlashWaitBusy(W25Q128JV_SECTOR_ERASE_MAX_TIME);
...
bool spiFlashEraseSector(uint32_t sector_addr)
{
  ...
  ret &= spiFlashWaitBusy(W25Q128JV_SUBSECTOR_ERASE_MAX_TIME);
...
bool spiFlashWaitBusy(uint32_t timeout)
{
  ...
  while(1)
  {
    ...
    if (ret == true)
    {
      ...
      busy_bit = rx_buf[1] & W25Q128JV_SR_WIP;
      if (busy_bit == 0)
      {
        break;
      }
